export const allCourses = {
  1: {
    id: 1,
    title: "What Is Generative Artificial Intelligence?",
    instructor: "Pinar Seyhan Demirdag",
    duration: "1h 3m",
    badge: "Popular",
    badgeColor: "bg-purple-100 text-purple-800",
    image: "bg-gradient-to-br from-purple-500 to-pink-500",
    preview: "Learn the fundamentals of generative AI and how it's transforming industries...",
    chapters: [
      {
        id: 1,
        title: "Introduction",
        lessons: [
          {
            id: 1,
            title: "What is Generative AI?",
            duration: "5m 23s",
            completed: false,
            videoUrl: "https://www.youtube.com/watch?v=1ukSR1GRtMU&list=PL4cUxeGkcC9jLYyp2Aoh6hcWuxFDX6PBJ",
            description: "Understand the fundamentals of generative artificial intelligence and its applications across various fields.",
            objectives: [
              "Define generative AI and its key characteristics.",
              "Identify common applications of generative AI.",
              "Distinguish GenAI from traditional AI."
            ],
            transcript: [
              {
                timestamp: "0:00",
                text: "Welcome to the Generative AI course. Let's dive into this exciting topic."
              },
              {
                timestamp: "0:15",
                text: "Generative AI is a field that focuses on creating new content, like text, images, and video."
              },
              {
                timestamp: "0:30",
                text: "It's different from traditional AI, which is often used for classification or prediction."
              }
            ],
            resources: [
              {
                id: "1-mcq",
                type: "MCQ",
                question: `uydkdu ysñhka úiska rpkd lrk ,oezhs ie,flk › ,xldfõ foaYmd,k b;sydih ms<sn| iúia;r f;dr;=re wvx.= jxYl;dj l=ulao@
(1) §mjxYh
(2) uydjxYh
(3) odGdjxYh
(4) fndaê jxYh`,
                answer: "විකල්පය 2"
              }
            ]
          },
          {
            id: 2,
            title: "Understanding Foundation Models",
            duration: "8m 45s",
            completed: false,
            videoUrl: "https://www.youtube.com/watch?v=bKueYVtV0eA&list=PL4cUxeGkcC9jLYyp2Aoh6hcWuxFDX6PBJ&index=2",
            description: "Explore the concept of foundation models and their role as the building blocks of modern GenAI systems.",
            objectives: [
              "Explain what a foundation model is.",
              "Describe the multi-tasking capabilities of foundation models.",
              "Recognize the importance of minimal training for targeted use cases."
            ],
            transcript: [
              {
                timestamp: "0:00",
                text: "Foundation models are the core of modern generative AI."
              },
              {
                timestamp: "0:10",
                text: "These large models are pre-trained on a massive amount of data."
              },
              {
                timestamp: "0:25",
                text: "This allows them to perform a wide variety of tasks with minimal fine-tuning."
              }
            ],
            resources: [
              {
                id: "2-mcq",
                type: "MCQ",
                question: `› ,xldj .ek f;dr;=re iemhQ úfoaY f,aLlhka lsysmfofkl= I ;Srefõ o" Tjqka wh;a jQ rgj,a II ;Srefõ o oelafõ' ;Srej II ;Srej yd .e<mQ úg ksjerÈ ms<s;=r jkafka ñka l=ula o@

I ;Srej   II ;Srej
(i) bnka n;+;d    (A) wrdìh
(ii) *dyshka       (B) bkaÈhdj
(iii) fl!á,H       (C) nqreuh (ñhkaud¾)
                 (D) Ökh

(1) AD iy B
(2) CD iy A
(3) AC iy B
(4) DA iy B`,
                answer: "විකල්පය 1"
              }
            ]
          },
          {
            id: 3,
            title: "Generative AI Use Cases",
            duration: "7m 12s",
            completed: false,
            videoUrl: "https://www.youtube.com/watch?v=LtlsX_lCfK4&list=PL4cUxeGkcC9jLYyp2Aoh6hcWuxFDX6PBJ&index=3",
            description: "Explore practical generative AI deployments across marketing, design, product, and analytics teams.",
            objectives: [
              "Recognize recurring generative AI use cases across industries.",
              "Map each scenario to the underlying model capabilities it requires.",
              "Surface risks, stakeholders, and success metrics ahead of pilots."
            ],
            transcript: [
              {
                timestamp: "0:00",
                text: "We'll highlight the most common generative AI use cases teams ship today."
              },
              {
                timestamp: "0:13",
                text: "From marketing copy to design assets, you'll see how the same foundation models adapt."
              },
              {
                timestamp: "0:28",
                text: "We'll wrap with a checklist for evaluating whether a workflow is mature enough for GenAI."
              }
            ],
            resources: [
              {
                id: "3-mcq",
                type: "MCQ",
                question: `wkqrdOmqr hq.fha wdrdu md,kh ms<sn| f;dr;=re we;=<;a jkafka ñka ljrlo@
(1) nÿ¨ geï ,smsh
(2) is;=,amõj fi,a ,smsh
(3) ñyska;,d mqjre ,smsh
(4) f.dvjdh geï ,smsh`,
                answer: "විකල්පය 3"
              }
            ]
          },
          {
            id: 4,
            title: "Model Capabilities and Limits",
            duration: "6m 54s",
            completed: false,
            videoUrl: "https://www.youtube.com/watch?v=FLQ-Vhw1NYQ&list=PL4cUxeGkcC9jLYyp2Aoh6hcWuxFDX6PBJ&index=4",
            description: "Understand what modern foundation models excel at and where they still struggle.",
            objectives: [
              "Differentiate between generative strengths like reasoning, summarization, and style transfer.",
              "Spot limitations around factual accuracy, bias, and interpretability.",
              "Plan mitigation strategies before deploying models in production."
            ],
            transcript: [
              {
                timestamp: "0:00",
                text: "Every generative model has signature strengths."
              },
              {
                timestamp: "0:14",
                text: "We'll analyze limits like hallucinations and long-context drift."
              },
              {
                timestamp: "0:32",
                text: "You'll leave with mitigation patterns you can use in evaluations."
              }
            ],
            resources: [
              {
                id: "4-mcq",
                type: "MCQ",
                question: `ls;a ;aisßfujka rcqf.a hehs ie,flk f;da‚., Ys,d,smsfha w;S; lDIsl¾uh .ek f;dr;=re we;=<;a fõ' fuys —msg ovyi —" —wl,yi —" — uefoyi — hkqfjka i|yka j we;af;a ljrla úh yelso@
(1) c, iïmdokh iïnkaO nÿ j¾. ;=kls'
(2) lDIsl¾uhg iïnkaO rdcH ks,Orhka ;sfofkls'
(3) j.d l< OdkH j,ska wh l< nÿ j¾. ;=kls'
(4) ù j.djg iïnkaO j.d lkak ;=kls'`,
                answer: "විකල්පය 4"
              }
            ]
          },
          {
            id: 5,
            title: "Getting Started Toolkit",
            duration: "4m 48s",
            completed: false,
            videoUrl: "https://www.youtube.com/watch?v=TSIhiZ5jRB0&list=PL4cUxeGkcC9jLYyp2Aoh6hcWuxFDX6PBJ&index=5",
            description: "Assemble the minimum viable toolkit to prototype generative AI ideas quickly.",
            objectives: [
              "Select hosted model providers, SDKs, and notebooks for experimentation.",
              "Set up prompt libraries, evaluation sheets, and dataset trackers.",
              "Share a lightweight governance checklist with collaborators."
            ],
            transcript: [
              {
                timestamp: "0:00",
                text: "Before building, we need a toolkit that keeps experiments organized."
              },
              {
                timestamp: "0:12",
                text: "We'll compare hosted playgrounds, SDKs, and workflows for quick iteration."
              },
              {
                timestamp: "0:27",
                text: "Finally, we'll add tracking templates that keep experiments repeatable."
              }
            ],
            resources: [
              {
                id: "5-mcq",
                type: "MCQ",
                question: `wkqrdOmqr hq.fha§ bkaÈhdkq ixl%u‚lhka yd wdl%u‚lhka › ,xldjg f.dv neiSu i|yd nyq,j Ndú; lr we;s f;dgqm< l=ulao@
(1) Wrefõ,
(2) f.dalK
(3) f.dvjdh
(4) uyd;s;a:`,
                answer: "විකල්පය 4"
              }
            ]
          }
        ]
      },
      {
        id: 2,
        title: "Core Concepts",
        lessons: [
          {
            id: 6,
            title: "Training AI Models",
            duration: "12m 30s",
            completed: false,
            videoUrl: "https://www.youtube.com/watch?v=B5Na9Ehd0QU",
            description: "Learn the process of training generative AI models, from data preparation to fine-tuning.",
            objectives: [
              "Outline the steps involved in training an AI model.",
              "Understand the role of supervised and unsupervised learning.",
              "Discuss the importance of data quality."
            ],
            transcript: [
              {
                timestamp: "0:00",
                text: "Training a generative AI model can be broken down into several key steps."
              },
              {
                timestamp: "0:15",
                text: "We'll explore data collection and preprocessing in detail."
              },
              {
                timestamp: "0:30",
                text: "You'll see how training and evaluation metrics are chosen."
              }
            ],
            resources: [
              {
                id: "6-mcq",
                type: "MCQ",
                question: `jir y;<sia y;rla idOdrK yd kS;s .rel md,khla f.k .shd hehs ie,flk wkqrmqr øúv md,lhd ljqo@
(1) t<dr
(2) ;sÍ;r
(3) mq,y;a:
(4) mkhudr`,
                answer: "විකල්පය 1"
              }
            ]
          },
          {
            id: 7,
            title: "Generative vs. Discriminative",
            duration: "15m 22s",
            completed: false,
            videoUrl: "https://www.youtube.com/watch?v=0OI2CKbneHc",
            description: "Differentiate between generative and discriminative AI models and their specific use cases.",
            objectives: [
              "Articulate the core difference between generative and discriminative models.",
              "Provide examples for both model types.",
              "Select the appropriate model for a given problem."
            ],
            transcript: [
              {
                timestamp: "0:00",
                text: "What's the real difference between generative and discriminative models?"
              },
              {
                timestamp: "0:12",
                text: "Generative models focus on how the data is generated."
              },
              {
                timestamp: "0:24",
                text: "Discriminative models focus on drawing boundaries between classes."
              }
            ],
            resources: [
              {
                id: "7-mcq",
                type: "MCQ",
                question: `m<uq jk úchndyq rc;=udf.ka isÿ jQ fiajdj we.hSfï§ m%uqL;ajh Èh hq;af;a l=uk lreKgo@
(1) wNdjhg f.dia ;snQ jdßud¾. m%;sixialrKh lsÍu
(2) {d;s iïnkaO;d u.ska wi,a jeis rdcH iuÕ iqyoj lghq;= lsÍu
(3) fid<S md,kfhka furg uqojd f.k iajdëk rdcHhla ìys lsÍu
(4) kgnqka j ;snQ fjfyr úydr m%;sixialrKh lsÍu`,
                answer: "විකල්පය 3"
              }
            ]
          },
          {
            id: 8,
            title: "Ethical Considerations",
            duration: "9m 18s",
            completed: false,
            videoUrl: "https://www.youtube.com/watch?v=pidy886cj9I",
            description: "Examine the ethical challenges and societal impacts of widespread generative AI adoption.",
            objectives: [
              "Recognize potential biases in AI training data.",
              "Discuss the concept of intellectual property in GenAI.",
              "Evaluate the societal impact of automation and misinformation."
            ],
            transcript: [
              {
                timestamp: "0:00",
                text: "Ethics in AI is more important than ever."
              },
              {
                timestamp: "0:08",
                text: "We'll discuss data privacy and algorithmic transparency."
              },
              {
                timestamp: "0:18",
                text: "You'll also see real-world case studies."
              }
            ],
            resources: [
              {
                id: "8-mcq",
                type: "MCQ",
                question: `w;sßla; c,h msg lsÍu i|yd ;kd we;s ks¾udKh oelafjkafka ljr wlaIrfhlao@
(1) A
(2) B
(3) C
(4) D`,
                answer: "විකල්පය 1"
              }
            ]
          },
          {
            id: 9,
            title: "Data Curation Essentials",
            duration: "11m 06s",
            completed: false,
            videoUrl: "https://www.youtube.com/watch?v=eMvo2i_Q4N8",
            description: "Curate datasets that keep generative AI outputs high quality and safe.",
            objectives: [
              "Audit source data for coverage, recency, and licensing.",
              "Design labeling and review workflows that scale with demand.",
              "Monitor data quality drift and schedule retraining triggers."
            ],
            transcript: [
              {
                timestamp: "0:00",
                text: "Data curation makes or breaks a generative project."
              },
              {
                timestamp: "0:16",
                text: "We'll cover sourcing, labeling, and human review strategies."
              },
              {
                timestamp: "0:33",
                text: "You'll get a maintenance plan for keeping datasets fresh."
              }
            ],
            resources: [
              {
                id: "9-mcq",
                type: "MCQ",
                question: `fuys C j,ska oelafjk ks¾udKfha m%Odk ld¾hh l=ulalao@
(1) jejg tl;= jk frdka uv md,kh lsÍu
(2) jefjys c, mSvkh iukh fldg m%Odk we<g c,h ksl=;a lsÍu
(3) jefjys r< myßka jeõ neïu fia§ hdu je<elaùu
(4) kshx ld, j,§ iSñ; c, m%udKhla l=Uqre j,g uqod yeÍu`,
                answer: "විකල්පය 3"
              }
            ]
          },
          {
            id: 10,
            title: "Prompt Patterns",
            duration: "8m 19s",
            completed: false,
            videoUrl: "https://www.youtube.com/watch?v=5Q1phdX5GxY",
            description: "Build a prompt pattern library that speeds up experimentation and collaboration.",
            objectives: [
              "Group prompts by task families like extraction, transformation, and generation.",
              "Compose role, context, and constraint patterns for complex workflows.",
              "Track experiment results so the best prompts become reusable templates."
            ],
            transcript: [
              {
                timestamp: "0:00",
                text: "Prompt patterns turn ad-hoc experiments into a system."
              },
              {
                timestamp: "0:18",
                text: "We'll remix role, context, and constraint techniques."
              },
              {
                timestamp: "0:35",
                text: "You'll also learn how to measure and store winning prompts."
              }
            ],
            resources: [
              {
                id: "10-mcq",
                type: "MCQ",
                question: `;=,dNdr odk §u" rg ;=< ixpdrh lsÍu yd Ys,d,sms úYd, ixLHdjla lrùu u.ska jeishka w;r ckm%sh ùfï m%;sm;a;shla wkq.ukh l< fmdf<dkakre md,lhd ljqo@
(1) m<uqjk úchndyq rc;=ud
(2) m<uqjk mrdl%undyq rc;=ud
(3) idyiu,a, rc;=ud
(4) lS¾;s › ksYaYxlu,a, rc;=ud`,
                answer: "විකල්පය 4"
              }
            ]
          }
        ]
      },
      {
        id: 3,
        title: "Advanced Techniques",
        lessons: [
          {
            id: 11,
            title: "Prompt Engineering Deep Dive",
            duration: "11m 04s",
            completed: false,
            videoUrl: "https://www.youtube.com/watch?v=67HM9d1gjpY",
            description: "Master prompt structures and troubleshooting tactics to reliably steer large language models.",
            objectives: [
              "Spot prompt patterns that produce inconsistent outputs.",
              "Apply role, context, and constraint prompting for complex tasks.",
              "Iterate prompts efficiently using model feedback."
            ],
            transcript: [
              {
                timestamp: "0:00",
                text: "Prompt engineering is the craft of steering models intentionally."
              },
              {
                timestamp: "0:20",
                text: "We will cover role prompting, constraints, and examples."
              },
              {
                timestamp: "0:45",
                text: "Finally, we'll fix common prompt failure modes."
              }
            ],
            resources: [
              {
                id: "11-mcq",
                type: "MCQ",
                question: `my; i|yka isÿùï w;=frka l=reKE., rdcOdks iufha ,xldjg meñ‚ úfoaYslhl=g wikakg fyda olskakg ,efnk isÿùï we;=<;a ms<s;=r l=ulao@
A.fojk mrdl%undyq rc;=ud úiska cdjl pkaøNdkq mrdch lsÍu
B.hdmkfha md,lhd f,i imqu,a l=ure lghq;= lsÍu
C.miajk ñys÷ rc isrlrejl= f,i bkaÈhdjg f.k hdu
D.NslaIQkaf.a úkh kS;s ms<sn| oUfo‚ l;sldj; iïmdokh lsÍu
E.fj!fyda fikam;shd ,xldj wdl%uKh lsÍu

(1) AC iy D
(2) CD iy E
(3) BC iy D
(4) ADiy E`,
                answer: "විකල්පය 1"
              }
            ]
          },
          {
            id: 12,
            title: "Fine-Tuning Foundation Models",
            duration: "13m 18s",
            completed: false,
            videoUrl: "https://www.youtube.com/watch?v=dPrs8Uz0wzE",
            description: "See how transfer learning and parameter-efficient methods adapt large models to niche domains.",
            objectives: [
              "Compare full fine-tuning with adapter-based techniques like LoRA.",
              "Set up a reproducible fine-tuning workflow.",
              "Validate fine-tuned models against domain-specific metrics."
            ],
            transcript: [
              {
                timestamp: "0:00",
                text: "Fine-tuning adapts a general model to your specific domain."
              },
              {
                timestamp: "0:18",
                text: "We'll compare full fine-tuning with parameter-efficient techniques."
              },
              {
                timestamp: "0:40",
                text: "Evaluation ensures the tuned model meets quality goals."
              }
            ],
            resources: [
              {
                id: "12-mcq",
                type: "MCQ",
                question: `› ,dxlsl jdia;= úoHdj yd uQ¾;s Ys,amh flfrys lemS fmfkk yskaÿ n,mEï oelsh yelafla ljr hq.j, §o@
(1) wkqrdOmqr - oUfo‚
(2) fmdf<dkakre - .ïfmd<
(3) hdmyq - l=reKE.,
(4) uykqjr - fldaÜfÜ`,
                answer: "විකල්පය 2"
              }
            ]
          },
          {
            id: 13,
            title: "Evaluating Generative Outputs",
            duration: "10m 42s",
            completed: false,
            videoUrl: "https://www.youtube.com/watch?v=Lt9-7Y_KZs0",
            description: "Combine automated metrics and human review to assess the quality of generative results.",
            objectives: [
              "Select qualitative and quantitative evaluation methods.",
              "Design human review workflows for generative content.",
              "Build scorecards that track model regressions over time."
            ],
            transcript: [
              {
                timestamp: "0:00",
                text: "Evaluation protects against regressions in generative quality."
              },
              {
                timestamp: "0:14",
                text: "Blend automatable metrics with expert human reviews."
              },
              {
                timestamp: "0:32",
                text: "We'll close with a sample evaluation checklist."
              }
            ],
            resources: [
              {
                id: "13-mcq",
                type: "MCQ",
                question: `wk.dßl O¾umd," wdreuq. kdj,¾" isoaÈ f,ífí hk mqoa.,hkaf.a lghq;= j, lemSfmfkk fmdÿ ,laIKh l=ulao@
(1) cd;sl iuÕsh j¾Okh lsÍu i|yd iduQyslj lghq;= lsÍuh'
(2) iqrdie,a ;ykï lsÍfï jHdmdrh jHdma; lsÍug lghq;= lsÍuh'
(3) ì%;dkH md,kfhka › ,xldj uqojd .ekSu i|yd lghq;= lsÍu h'
(4) iajlSh wd.u yd ixialD;sh ms<sn| mqk¾Ôjhla we;s lsÍug lghq;= lsÍuh'`,
                answer: "විකල්පය 4"
              }
            ]
          },
          {
            id: 14,
            title: "Reinforcement Learning from Human Feedback",
            duration: "14m 05s",
            completed: false,
            videoUrl: "https://www.youtube.com/watch?v=3PjO1qX8kUo",
            description: "Understand how RLHF aligns model behaviour with human expectations and safety guardrails.",
            objectives: [
              "Summarize the RLHF training loop.",
              "Collect preference data to train reward models.",
              "Integrate feedback updates into iterative model releases."
            ],
            transcript: [
              {
                timestamp: "0:00",
                text: "RLHF teaches models to follow human preferences."
              },
              {
                timestamp: "0:22",
                text: "We'll gather comparison data to train a reward model."
              },
              {
                timestamp: "0:48",
                text: "Policy optimization aligns the base model with the reward."
              }
            ],
            resources: [
              {
                id: "14-mcq",
                type: "MCQ",
                question: `ì%;dkHhkag › ,xldj flfrys wjOdkh fhduq lsÍug fya;= jQ lreK l=ulao@
(1) j;= j.dj wdrïN lsÍug iqÿiq bvï ,nd.ekSfï wjYH;dj
(2) foaYSh ck;dj l%sia;shdks wd.ug yrjd .ekSfï wNs,dIh
(3) bkaÈhka id.rfha kdjql lghq;= fufyh ùug ;%sl=Kdu,h jrdfha we;s jeo.;alu jgyd .ekSu
(4) › ,xldfõ l=re÷ fj<|du ms<sn| tAldêldrh ,nd.ekSfï wjYH;dj`,
                answer: "විකල්පය 3"
              }
            ]
          },
          {
            id: 15,
            title: "Optimizing Inference Pipelines",
            duration: "9m 56s",
            completed: false,
            videoUrl: "https://www.youtube.com/watch?v=Yr1yIgiZHiM",
            description: "Reduce cost and latency of GenAI workloads with hardware, batching, and caching strategies.",
            objectives: [
              "Profile inference bottlenecks across hardware choices.",
              "Apply batching, caching, and streaming to reduce latency.",
              "Estimate serving costs and plan autoscaling policies."
            ],
            transcript: [
              {
                timestamp: "0:00",
                text: "Serving GenAI models efficiently is a cost challenge."
              },
              {
                timestamp: "0:16",
                text: "We'll profile hardware choices and batching strategies."
              },
              {
                timestamp: "0:34",
                text: "Monitoring helps you react to spikes in demand."
              }
            ],
            resources: [
              {
                id: "15-mcq",
                type: "MCQ",
                question: `› ,xldj hg;a úð;hla njg m;alrk ,oafoa my; i|yka ljr fldñiul fhdackdjla u; o@
(1) o ñhqrka fldñiu
(2) flda,anDDla fldñiu
(3) fvdkfuda¾ fldñiu
(4) fida,anß fldñiu`,
                answer: "විකල්පය 1"
              }
            ]
          }
        ]
      },
      {
        id: 4,
        title: "Building With GenAI",
        lessons: [
          {
            id: 16,
            title: "Designing Conversational Agents",
            duration: "12m 12s",
            completed: false,
            videoUrl: "https://www.youtube.com/watch?v=1d9kA0v1iW0",
            description: "Plan natural, on-brand conversational flows that keep users engaged and supported.",
            objectives: [
              "Map user intents into conversational flows.",
              "Craft prompt templates that maintain context and tone.",
              "Measure and improve bot satisfaction metrics."
            ],
            transcript: [
              {
                timestamp: "0:00",
                text: "Conversational design starts with intent mapping."
              },
              {
                timestamp: "0:18",
                text: "We'll balance automation with human hand-offs."
              },
              {
                timestamp: "0:37",
                text: "Tone and personality matter for user trust."
              }
            ],
            resources: [
              {
                id: "16-mcq",
                type: "MCQ",
                question: `l%s'j' 1818 § yd l%s'j' 1848 § › ,xldfõ we;s jQ ksoyia wr., yd iïnkaO f;dr;=re my; A, B, C, D, E hgf;a olajd we;' tAjd w;=frka l%s'j' 1818 ksoyia wr.,h yd iïnkaO f;dr;=re muKla wvx.= ms<s;=r f;darkak'
A.lemafmá fmd< Èidfõf.a kdhl;ajh
B.fgdßx.agka idñf.a nÿ m%;sm;a;sh
C.f.dx.df,af.dv nKavd isxyf,a rcq jYfhka wNsfIal ,eîu
D.fud,a,sf.dv wÈldrïf.a l%shdl,dmh
E.yÊð ußlald¾ fj,a,iafia uqykaÈrï f,i m;a lrkq ,eîu

(1) BCD
(2) ACE
(3) ADE
(4) CDE`,
                answer: "විකල්පය 3"
              }
            ]
          },
          {
            id: 17,
            title: "Workflow Automation with AI",
            duration: "8m 58s",
            completed: false,
            videoUrl: "https://www.youtube.com/watch?v=ISp8pPYfacs",
            description: "Automate repetitive knowledge work by chaining GenAI capabilities with existing tools.",
            objectives: [
              "Identify high-value automation candidates.",
              "Orchestrate GenAI services within existing tools.",
              "Monitor automations for drift and compliance."
            ],
            transcript: [
              {
                timestamp: "0:00",
                text: "Automation frees teams to focus on creative work."
              },
              {
                timestamp: "0:15",
                text: "We'll orchestrate GenAI calls inside business workflows."
              },
              {
                timestamp: "0:33",
                text: "Auditing ensures automations stay compliant."
              }
            ],
            resources: [
              {
                id: "17-mcq",
                type: "MCQ",
                question: `ì%;dkH md,k iuh ;=< furg isÿ lrk ,o wdKavql%u jHjia:d ixfYdaOkj, olakg ,enqKq j¾;udkfha mjd wys;lr n,mEï we;s lsÍug iu;a jQ ,laIKh f,i ie,lsh yelafla"
(1) ks,,;a uka;%Ska ixLHdjg jvd ks, fkd,;a uka;%Ska ixLHdj l%ufhka jeäùuhs'
(2) ckj¾. wkqj uka;%Sjreka m;a lsÍuhs'
(3) rdcH ks,OdÍkaf.a n,h uyck ksfhdað;hkaf.a n,h wNsnjd hEuhs'
(4) kdu lrKfhka uka;%Sjreka m;alsÍug wdKavqldrhdg n,;, ;sîuhs'`,
                answer: "විකල්පය 2"
              }
            ]
          },
          {
            id: 18,
            title: "Integrating GenAI into Products",
            duration: "16m 11s",
            completed: false,
            videoUrl: "https://www.youtube.com/watch?v=dW7Z2I9srJs",
            description: "Partner with cross-functional teams to ship generative experiences users love.",
            objectives: [
              "Define product principles for generative experiences.",
              "Collaborate with legal, design, and engineering stakeholders.",
              "Plan rollout strategies with user feedback loops."
            ],
            transcript: [
              {
                timestamp: "0:00",
                text: "Product integration requires collaboration."
              },
              {
                timestamp: "0:17",
                text: "We'll examine discovery, prototyping, and launch playbooks."
              },
              {
                timestamp: "0:38",
                text: "User feedback loops keep features improving."
              }
            ],
            resources: [
              {
                id: "18-mcq",
                type: "MCQ",
                question: `fida,anß wdKavql%u m%;sixialrK j,g wod<j › ,xldfõ we;s l< fjkialula jkafka ñka l=ulao@
(1) úOdhl ldrl iNd msysgqùuh'
(2) rdcH uka;%K iNdj msysgqùuh'
(3) i¾jck Pkao n,h ,nd §uh'
(4) W;a;r uka;%S uKav,h ia:dms; lsÍu h'`,
                answer: "විකල්පය 4"
              }
            ]
          },
          {
            id: 19,
            title: "Responsible Deployment Checklist",
            duration: "7m 47s",
            completed: false,
            videoUrl: "https://www.youtube.com/watch?v=0Thw5Z3n6zA",
            description: "Follow a clear checklist covering privacy, transparency, and support before launch.",
            objectives: [
              "Audit datasets and licensing before launch.",
              "Create transparent user communications.",
              "Establish review cadences for post-launch oversight."
            ],
            transcript: [
              {
                timestamp: "0:00",
                text: "Before launch, run through a responsible AI checklist."
              },
              {
                timestamp: "0:16",
                text: "We'll validate data sources and document capabilities."
              },
              {
                timestamp: "0:34",
                text: "Communications help users understand boundaries."
              }
            ],
            resources: [
              {
                id: "19-mcq",
                type: "MCQ",
                question: `ì%;dkH md,k iuh ;=<§ j;=j.dfõ ÈhqKqjg fukau foaYSh lDIsl¾ufha mßydkshg o tlfia n,mE fya;=jla jkafka"
(1) wksjd¾h fiajd rdcldÍ l%uh wfydais lsÍu h'
(2) uqvq ìï mk; iïu; lsÍu h'
(3) bkaÈhdkq lïlrejkaf.a Y%uh ,nd .ekSu h'
(4) isú,a fiajlhkag fm!oa.,sl jHdmdr j, fh§ug bv ie,iSuh'`,
                answer: "විකල්පය 2"
              }
            ]
          },
          {
            id: 20,
            title: "Monitoring Model Performance",
            duration: "9m 02s",
            completed: false,
            videoUrl: "https://www.youtube.com/watch?v=9lMG-tgRNi0",
            description: "Track quality, safety, and usage signals so deployed models keep getting better.",
            objectives: [
              "Set up telemetry for quality, usage, and safety signals.",
              "Alert teams when anomalies occur.",
              "Capture user feedback to drive model improvements."
            ],
            transcript: [
              {
                timestamp: "0:00",
                text: "Monitoring is ongoing work once a model is live."
              },
              {
                timestamp: "0:17",
                text: "We'll instrument dashboards for quality and safety."
              },
              {
                timestamp: "0:36",
                text: "Feedback loops feed future model improvements."
              }
            ],
            resources: [
              {
                id: "20-mcq",
                type: "MCQ",
                question: `my; i|yka tAjdhska ngysr cd;Skaf.a n,mEu ksid › ,dxlsl iudchg tl;= jQ ,laIK muKla ksjerÈ j we;=<;a jkafka lskï ms<s;=fra o@
A.nhs,d - lmsß[a[d ix.S;h
B.f;aij,dfuhs kS;sh
C.i¾jck Pkao n,h
D.rdcldß l%uh
E.wdhq¾fõo ffjoH l%uh
F.frmrudÿ wd.u

(1) ABC
(2) BDE
(3) ACF
(4) CEF`,
                answer: "විකල්පය 3"
              }
            ]
          }
        ]
      }
    ]
  }
};
